...
def creat_start(calc_density):
    ...
    rows,cols = 2500, 2500 #have to be sqaured
...
def main(heels_desity, fronts_desity,heel_pos_x, heel_pos_y, rows, cols, POS, starting_pos_x,starting_pos_y, radius_fronts_avg):
    ...
    way_matrix_x = np.zeros((110*6,21))
    way_matrix_y = np.zeros((110*6,21))
    step_size_fil = np.zeros((110*6,20))
    step_size_stiff = np.zeros((110*6,20))
    fil_matrix_x = np.zeros((110*6,10))
    fil_matrix_y = np.zeros((110*6,10))
...
if __name__ == '__main__':
    ...
    making_movie = False
    folder_path = f"./modell_tanh_stiffness_bigger_grid/"
    nr_of_rec = 110 #number of bundles
