...
def creat_start(calc_density):
    ...
    range_v2 = np.arange(10)
    v1_x = 0
    for count_v1 in np.arange(11):
    ...
    #remove index:
    rm_index = np.arange(20*6)
    for index in np.arange(20*6,90*6,10*6):
        
        rm_index = np.append(rm_index,np.arange(index,index+12,1))
        rm_index = np.append(rm_index,np.arange(index+8*6,index+10*6,1))
    rm_index = np.append(rm_index,np.arange(90*6,110*6,1))
    border_index = list(set(np.arange(110*6))-set(rm_index))

    if calc_density:
        heels_desity = kernel_parabola(1, POS[0],heel_pos_x,radius_heels_avg,POS[1],heel_pos_y)
        bun_border_x = np.delete(heel_pos_x,border_index)
        bun_border_y = np.delete(heel_pos_y,border_index)
        heel_pos_x = np.delete(heel_pos_x,rm_index)
        heel_pos_y = np.delete(heel_pos_y,rm_index)
        heels_desity += kernel_parabola(0.5, POS[0],bun_border_x,radius_fronts_avg,POS[1],bun_border_y)
...
if __name__ == '__main__':
    ...
    making_movie = True
    folder_path = f"./modell_tanh_stiffness_border_bundles_fronts_added/"
    ...
    for sweeps in range(10):
        with open(f"{folder_path}3_rt_21_b_test_{sweeps}.npy", 'rb') as f:
                    way_matrix_x = np.load(f)
                    way_matrix_y = np.load(f)
                    grid_x = np.load(f)
                    grid_y = np.load(f)
        ...
        for R in range(42*6):
            plt.plot(way_matrix_x[R,:4],way_matrix_y[R,:4],color=["blue","green","red","yellow","pink","orange"][np.mod(R,6)])
